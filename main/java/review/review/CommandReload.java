...
public class CommandReload implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof ConsoleCommandSender == true) {
            main.reloadConfig();
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', main.getConfig().getString("reload")));
        }else {
            //Теперь если игрок
            ...
            if (executor.hasPermission("review.reload") == false) {
                executor.sendMessage(ChatColor.translateAlternateColorCodes('&', main.getConfig().getString("noperms")));
                return true;
            }
